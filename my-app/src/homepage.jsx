...
import { Menu, X, Sun, Moon, CheckSquare, Users, PieChart } from 'lucide-react';
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-16">
                    <div className="flex items-center">
                        <span onClick={() => navigate('/')} className="font-bold text-2xl text-indigo-600 cursor-pointer">SynergySphere</span>
                    </div>
                    <div className="hidden md:block">
                        <div className="ml-10 flex items-center space-x-4">
                            <ThemeToggle theme={theme} toggleTheme={toggleTheme} />
                            <button onClick={() => navigate('/login')} className="text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white px-3 py-2 rounded-md text-sm font-medium transition-colors">Login</button>
        ...
        <section className="bg-white dark:bg-gray-900 pt-24 md:pt-32 transition-colors duration-300">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
                ...
                <div className="mt-8 flex justify-center">
                    <button onClick={() => navigate('/signup')} className="px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 md:py-4 md:text-lg md:px-10 transition-transform transform hover:scale-105 duration-300">Get Started</button>
                </div>
            </div>
        </section>
    );
    ...
    <section className="py-20 bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <FeatureCard icon={<CheckSquare className="h-12 w-12 text-indigo-500" />} title="Intuitive Task Management">Organize your work with assignees, due dates, and priorities. Track progress from start to finish.</FeatureCard>
                <FeatureCard icon={<Users className="h-12 w-12 text-indigo-500" />} title="Seamless Collaboration">Keep your team in sync with shared workspaces, real-time comments, and file sharing.</FeatureCard>
                <FeatureCard icon={<PieChart className="h-12 w-12 text-indigo-500" />} title="Insightful Reporting">Gain valuable insights with customizable dashboards and progress reports to make data-driven decisions.</FeatureCard>
            </div>
        </div>
    </section>
);
